...
每周一至周五北京时间 16:36 计划启动，19:36 再做一次兜底检查，时区固定为 `Asia/Shanghai`。这只是启动时间，不保证该分钟内完成日报。每次先核验交易日历和收盘时间；休市日或尚未收盘时，不生成新的盘后信号，也不以旧日期报告冒充当天结果。当日股票行情暂未覆盖交易日时，计算占位前最多再等待两次，每次 10 分钟后重新取数并核验；其他错误或重试后仍缺失则如实失败。兜底运行会复用已保存的当日结果，不重复计算。
